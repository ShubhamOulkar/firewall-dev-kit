...
if [ "$VERBOSE" = "true" ]; then
    ...
    # Fix for VS Code terminals: Non-login interactive shells do not source /etc/profile.d/
    # We must explicitly source it in the global bashrc and zshrc
    echo "source /etc/profile.d/firewall-verbose.sh" >> /etc/bash.bashrc
    
    mkdir -p /etc/zsh
    echo "source /etc/profile.d/firewall-verbose.sh" >> /etc/zsh/zshrc
